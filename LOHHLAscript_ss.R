...
BAMDir <- dirname(tumorBAMfile)
#print(coverageStep)
#print(plottingStep)

if (is.null(opt$outputDir) | is.null(opt$hlaPath) | is.null(opt$HLAfastaLoc)){
...
)
{
  ...
  if(!override){
    ...
    for(BAMfile in BAMfiles){
      region <- strsplit(BAMfile, '\\.')[[1]][1]
      cmd    <- paste('samtools flagstat ', paste(BAMDir, BAMfile, sep = '/'), ' > ', outDir, region, '.proc.flagstat', sep = '')
      system(cmd)
    }  
  }    
...
BAMfiles  <- c(basename(normalBAMfile), basename(tumorBAMfile))
BAMfiles_invariant <- c(basename(normalBAMfile), basename(tumorBAMfile))
normalregion <- strsplit(basename(normalBAMfile), '\\.')[[1]][1]

regions   <- sapply(BAMfiles, FUN = function(x) {return(strsplit(x, '\\.')[[1]][1])})
...
if(mapping.step){
  ...
  for(BAMfile in BAMfiles){
    ...
    BAMid <- strsplit(BAMfile, split = '\\.')[[1]][1]
    
    if(BAMid == normalregion){
      normalName <- BAMid
    }
...
normalName <- normalregion
# also extract number of unique reads sequenced in tumor and normal
if(runWithNormal){

  if(!override){
    regionUniqMappedRegions <- getUniqMapReads(workDir = workDir, BAMDir = BAMDir, BAMfiles = BAMfiles_invariant, override = FALSE)
  }
  if(override){
    regionUniqMappedRegions <- getUniqMapReads(workDir = workDir, BAMDir = BAMDir, BAMfiles = BAMfiles_invariant, override = TRUE, overrideDir = overrideDir) 
  }
